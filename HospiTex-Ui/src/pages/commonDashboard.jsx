import React from "react";
import { Link, useNavigate } from "react-router-dom";
import { Stethoscope, FileText, Users, CheckCircle, Clock, Shield, Mail, Phone } from "lucide-react";

const CommonDashBoard = () => {
  const navigate = useNavigate();

  const portals = [
    {
      title: "Patient Portal",
      description: "Access your health records, book appointments, and manage reports.",
      icon: <Users className="w-10 h-10 text-indigo-600" />,
      path: "/patient-dashboard",
      color: "from-indigo-100 to-indigo-200"
    },
    {
      title: "Doctor Portal",
      description: "Manage patient consultations, view medical history, and collaborate.",
      icon: <Stethoscope className="w-10 h-10 text-green-600" />,
      path: "/doctor-dashboard",
      color: "from-green-100 to-green-200"
    },
    {
      title: "Diagnostic Portal",
      description: "Upload lab reports, manage tests, and communicate with patients.",
      icon: <FileText className="w-10 h-10 text-purple-600" />,
      path: "/diagnostic-dashboard",
      color: "from-purple-100 to-purple-200"
    }
  ];

  const features = [
    { title: "Trusted Doctors", desc: "Certified and experienced doctors at your service.", icon: <CheckCircle className="w-8 h-8 text-green-600"/> },
    { title: "24/7 Service", desc: "Access medical support anytime, anywhere.", icon: <Clock className="w-8 h-8 text-indigo-600"/> },
    { title: "Secure Data", desc: "Your health records are private and encrypted.", icon: <Shield className="w-8 h-8 text-purple-600"/> },
  ];

  const contactInfo = {
    email: "support@hospitex.com",
    phone: "+1-234-567-8901"
  };

  return (
    <div className="min-h-screen bg-gradient-to-tr from-indigo-50 via-white to-green-50 flex flex-col">
      {/* Navbar */}
      <nav className="flex items-center justify-between px-8 py-4 bg-white shadow-md sticky top-0 z-50">
        <Link to="/" className="flex items-center gap-2">
          {/* <img src={logo} alt="HospiTex Logo" className="w-12 rounded-full" /> */}
          <span className="text-xl font-bold text-indigo-900">HospiTex</span>
        </Link>

        <button
          onClick={() => navigate('/login')}
          className="px-5 py-2 bg-indigo-600 text-white rounded-full shadow hover:bg-indigo-700 transition cursor-pointer"
        >
          Login / Sign Up
        </button>
      </nav>

      {/* Hero */}
      <header className="flex flex-col items-center justify-center py-20 px-6 text-center bg-gradient-to-br from-indigo-100 via-white to-green-100">
        <h1 className="text-5xl font-extrabold text-indigo-900">
          Welcome to HospiTex
        </h1>
        <p className="mt-4 text-lg text-gray-700 max-w-3xl">
          Your all-in-one healthcare platform connecting Patients, Doctors, and Diagnostic Centers.
          Book appointments, view reports, and manage your health easily and securely.
        </p>
        <button
          onClick={() => navigate('/login')}
          className="mt-6 px-8 py-3 bg-indigo-600 text-white rounded-full shadow hover:bg-indigo-700 transition cursor-pointer"
        >
          Get Started
        </button>
      </header>

      {/* Portal Cards */}
      <section className="grid grid-cols-1 md:grid-cols-3 gap-8 px-10 py-16">
        {portals.map(portal => (
          <Link
            to={portal.path}
            key={portal.title}
            className={`rounded-2xl shadow-lg p-8 flex flex-col items-center text-center bg-gradient-to-br ${portal.color} hover:shadow-2xl hover:scale-105 transition`}
          >
            <div className="mb-4">{portal.icon}</div>
            <h2 className="text-2xl font-semibold text-indigo-900">{portal.title}</h2>
            <p className="mt-2 text-gray-700 text-sm">{portal.description}</p>
            <span className="mt-4 text-indigo-600 font-medium">Enter →</span>
          </Link>
        ))}
      </section>

      {/* Features Section */}
      <section className="bg-white py-16 px-10">
        <h2 className="text-3xl font-bold text-center mb-10 text-indigo-900">Why Choose HospiTex?</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
          {features.map(feature => (
            <div key={feature.title} className="flex flex-col items-center gap-4 p-6 border rounded-xl hover:shadow-xl transition">
              {feature.icon}
              <h3 className="text-xl font-semibold">{feature.title}</h3>
              <p className="text-gray-600 text-sm">{feature.desc}</p>
            </div>
          ))}
        </div>
      </section>

      {/* Contact Section */}
      <section className="bg-indigo-50 py-12 px-10 text-center rounded-xl mx-10 mb-10 shadow-md">
        <h2 className="text-3xl font-bold mb-6 text-indigo-900">Get in Touch</h2>
        <p className="flex items-center justify-center gap-2 text-gray-700 mb-4">
          <Mail className="w-5 h-5 text-indigo-600" /> {contactInfo.email}
        </p>
        <p className="flex items-center justify-center gap-2 text-gray-700">
          <Phone className="w-5 h-5 text-indigo-600" /> {contactInfo.phone}
        </p>
      </section>

      {/* Footer */}
      <footer className="text-center py-8 text-gray-500 text-sm border-t bg-white">
        © {new Date().getFullYear()} HospiTex. All rights reserved.
      </footer>
    </div>
  );
};

export default CommonDashBoard;
